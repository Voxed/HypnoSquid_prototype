...
private:
    ...
    void Start() override
    {
        ...
        Bus().AddCallbacks(this, &TestSystem::Update, &TestSystem::Update2);
    ...
    {
        for (const auto &p : player->GetEntitiesWith())
            std::cout << "Player position again: { x: " << player->Get(p).x << ", y: " << player->Get(p).y << " }" << std::endl;
    }
};
...
auto main(int argc, const char **argv) -> int
{
    ...
    hs::EngineBuilder()
        .EmplaceAll<PhysicsSystem, TestSystem, hs::core::SystemMainLoop>()
        .Build()
        .Start();
}